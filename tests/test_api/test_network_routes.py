...
class TestGetAdapterStatus:
    """Tests for GET /api/network/adapter/{adapter_name}/status."""

    def test_status_parses_the_adapter_answer(self, client: TestClient) -> None:
        ps = _ps((True, ADAPTER_STATUS_JSON))
        with (
            _windows(),
            patch("fpstune.api.routes.system_network._run_powershell_async", new=ps),
        ):
            response = client.get("/api/network/adapter/Ethernet/status")

        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        assert data["adapter_name"] == "Ethernet"
        assert data["is_enabled"] is True
        assert data["is_connected"] is True
        assert data["adapter_type"] == "ethernet"
        assert data["ipv4_address"] == "192.168.1.42"
    ...
    def test_an_adapter_that_does_not_exist_is_404(self, client: TestClient) -> None:
        ...
        with (
            ...
            response = client.get("/api/network/adapter/Ghost/status")
    ...
    def test_an_unparseable_answer_is_500_not_a_crash(self, client: TestClient) -> None:
        """PowerShell answering something that is not JSON (an error banner, a
        localised warning) must surface as a clean 500, not an unhandled
        exception mid-response."""
        ps = _ps((True, "Get-NetAdapter : access denied"))
        with (
            _windows(),
            patch("fpstune.api.routes.system_network._run_powershell_async", new=ps),
        ):
